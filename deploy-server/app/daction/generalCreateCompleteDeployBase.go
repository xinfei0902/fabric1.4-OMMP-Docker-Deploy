...
var CliIP,CliName,completeDeployCliName string
...
func GeneralCreateCompleteDeployBase(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
	...
	ordererConnectHosts := make([]string,0)
	for _,orderer := range general.Orderer{
		ordererIntIP,err := dmysql.GetIntIPFromExtIP(orderer.IP)
		if err != nil{
			output.AppendLog(&objectdefine.StepHistory{
				Name:  "GeneralCreateCompleteDeployBase",
				Error: []string{err.Error()},
			})
			return err
		}
		//ordererConnectHosts = append(ordererConnectHosts,fmt.Sprintf("%s:%s", orderer.Domain, orderer.IP))
		ordererConnectHosts = append(ordererConnectHosts,fmt.Sprintf("%s:%s", orderer.Domain, ordererIntIP))
		peerConnectHosts = append(peerConnectHosts, fmt.Sprintf("%s:%s", orderer.Domain, ordererIntIP))
	}
	for _, orderer := range general.Orderer {
		...
		enviroment = append(enviroment, "ORDERER_KAFKA_TOPIC_REPLICATIONFACTOR=1")
	    enviroment = append(enviroment, "ORDERER_KAFKA_BROKERS=true")
		...
		ordererInfo.ExtraHosts = ordererConnectHosts
	...
	for _, org := range general.Org {
		...
		for _, peer := range org.Peer {
			...
			peerConnectHosts = append(peerConnectHosts, fmt.Sprintf("%s:%s", peer.Domain, peerIntIP))
	...
	for _, org := range general.Org {
		...
		cifeCaPath := filepath.ToSlash(filepath.Join(general.BaseOutput, "deploy","crypto-config/peerOrganizations", org.OrgDomain, "ca"))
		...
		cifeTLSPath := filepath.ToSlash(filepath.Join(general.BaseOutput, "deploy","crypto-config/peerOrganizations", org.OrgDomain, "tlsca"))
		...
		caInfo.Command = fmt.Sprintf("sh -c 'fabric-ca-server start -b admin:%s-admin -d'",org.Name)
		volumes := make([]string, 0)
		...
		volumes = append(volumes, fmt.Sprintf("./crypto-config/peerOrganizations/%s/tlsca:/etc/hyperledger/fabric-ca-server-config/tlsca", org.OrgDomain))
		...
		for _, peer := range org.Peer {
			...
			penviroment = append(penviroment, "CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=anhui_default")
			...
			//if len(peercli) == 0 {
				...
				completeDeployCliName = fmt.Sprintf("cli-%s-%s", org.Name, peer.Name)
				...
				clienvironment = append(clienvironment, "SYS_CHANNEL=byfn-sys-channel")
	...
	ret.Service = service
	data,err := yaml.Marshal(ret)	// 第二个表示每行的前缀，这里不用，第三个是缩进符号，这里用tab
	err = ioutil.WriteFile(filepath.Join(general.BaseOutput,"deploy","base.yaml"),data,0777)
	// err := tools.WriteYamlFile(filepath.Join(general.BaseOutput,"deploy", "base.yaml"), ret)
	if err != nil {
	...
	var cmd *exec.Cmd
	execCommand := fmt.Sprintf("cd %s && sed -i '1d' base.yaml && sed -i \"1iversion: '2'\" base.yaml",filepath.ToSlash(filepath.Join(general.BaseOutput, "deploy")))
