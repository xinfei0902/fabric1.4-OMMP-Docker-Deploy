...
//OperateAddOrgIP 用来在组织 cli端所在执行操作ip地址 因为现在部署只有一个cli 为以后考虑可能是其他cli所准备
...
func GeneralCreateOrgScriptFile(general *objectdefine.Indent, orgOrder objectdefine.OrgType, peerOrder objectdefine.PeerType, output *objectdefine.TaskNode) error {
	...
	ordererHosts := fmt.Sprintf("\"%s %s\"", ordererIntIP, ordererDomain)
	...
	change["peerHosts"] = fmt.Sprintf("(%s)", peerHostsList)
	//创建三个脚本文件
