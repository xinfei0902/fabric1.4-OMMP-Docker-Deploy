...
//###########一键部署##################
//MakeStepGeneralDeployZip 打包所有文件上传
func MakeStepGeneralCreateCompleteDeployZip(name string) objectdefine.RunTaskHandle {
	return func(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
		...
		err := GeneralCreateDeployZipFile(general, output)
		if err != nil {
			output.AppendLog(&objectdefine.StepHistory{
				Name:  name,
...
func GeneralCreateDeployZipFile(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
	base := filepath.Join(general.BaseOutput, "deploy")
	//base := general.BaseOutput
	outputRoot := general.BaseOutput
	f, err := os.Create(filepath.Join(outputRoot, "deploy"+".tar.gz"))
	if err != nil {
...
//GeneralUpgradeChainCodeZipFile 打包目录下所有文件
func GeneralDeployEnvZipFile(path string) error {
	base :=filepath.Join(path,"deployTools")
	f, err := os.Create(filepath.Join(path,"deployTools.tar"))
	if err != nil {
		err = errors.WithMessage(err, "Create tar.gz file error")
		return err
	}
	...
	err = filepath.Walk(base, func(path string, info os.FileInfo, err error) error {
		...
		zipPut := strings.TrimPrefix(path, path)
		// not zip base folder
		zipPut = strings.TrimLeft(zipPut, "\\/")
