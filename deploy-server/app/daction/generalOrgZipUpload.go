...
//######################意见部署###################################
//MakeStepGeneralDeployZipUpload 一键部署打包文件上传

func MakeStepGeneralCreateCompleteDeployZipUpload(name string) objectdefine.RunTaskHandle {
	return func(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
		...
		path := filepath.Join(general.BaseOutput, "deploy"+".tar.gz")
		for ip,_:= range general.FireWall{
			err := GeneralCreateDeployZipUploadFile(connectPort,path, ip,output)
			if err != nil {
				output.AppendLog(&objectdefine.StepHistory{
					Name:  name,
		...
		// err := GeneralCreateDeployZipUploadFile(general, output)
		// if err != nil {
		// 	output.AppendLog(&objectdefine.StepHistory{
		// 		Name:  name,
		// 		Error: []string{err.Error()},
...
//GeneralCreateDeployZipUploadFile 以后多组织扩展
func GeneralCreateDeployZipUploadFile(connectPort,path,ip string, output *objectdefine.TaskNode) error {
	
	//outputRoot := general.BaseOutput
	//path := filepath.Join(outputRoot, "deploy"+".tar.gz")
	//connectPort := dconfig.GetStringByKey("toolsPort")
	//FireWallByte,_ := json.Marshal(general.FireWall)
	//fmt.Println("deploy upload deploy.tar.gz",string(FireWallByte))
    //for ip,_:= range general.FireWall{
		fmt.Println("deploy upload deploy.tar.gz ip ==",ip)
		url := fmt.Sprintf("http://%s:%s/command/file/upload", ip, connectPort)
		output.AppendLog(&objectdefine.StepHistory{
			Name: "GeneralCreateDeployZipUploadFile",
...
//######################合约编译脚本上传###################################
func MakeStepGeneralChainCodeBulidScripUpload(name string) objectdefine.RunTaskHandle {
	return func(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
		output.AppendLog(&objectdefine.StepHistory{
			Name: name,
			Log:  []string{"create chainCode bulid script upload"},
		})
...
func MakeStepGeneralCreateCompleteDeployUnzip(name string) objectdefine.RunTaskHandle {
	return func(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
		output.AppendLog(&objectdefine.StepHistory{
			Name: name,
			Log:  []string{"create channel Unzip start"},
		...
		err := GeneralCreateCompleteDeployUnzipFile(general, output)
		if err != nil {
			output.AppendLog(&objectdefine.StepHistory{
				Name:  name,
				Error: []string{err.Error()},
...
//GeneralCreateDeployZipUploadFile 以后多组织扩展
func GeneralCreateCompleteDeployUnzipFile(general *objectdefine.Indent, output *objectdefine.TaskNode) error {
	
	connectPort := dconfig.GetStringByKey("toolsPort")
    for ip,_:= range general.FireWall{
		fmt.Println("deploy unzip deploy.tar.gz ip ==",ip)
		url := fmt.Sprintf("http://%s:%s/command/exec", ip, connectPort)
		output.AppendLog(&objectdefine.StepHistory{
			Name: "GeneralCreateCompleteDeployUnzipFile",
			Log:  []string{fmt.Sprintf("connect remote tools url:%s", url)},
		})

		command := "unpack"
		buff := fmt.Sprintf("mkdir -p deploy && tar -xvf %s.tar.gz -C ./deploy", "deploy")
		args := []string{buff}
		err := MakeHTTPRemoteCmd(url, command, args, output)
		if err != nil {
			output.AppendLog(&objectdefine.StepHistory{
