...
type ServerIpStruct struct {
	ServerIntIP string `json:"serverip,omitempty"`
...
func GetStartTaskBeforIndent(input *objectdefine.Indent) (*objectdefine.Indent, error) {
	...
	fmt.Println("indentSQL",indentSQL)
...
//GetIPListFromIndent 获取orderer ip列表和节点个数
func GetIPListFromOrderer() (map[string]int, error) {

	indentSQL := "select distinct orderer_ip from ommp_orderer"
	row, err := db.Query(indentSQL)
	defer row.Close()
	if err != nil {
		return nil, errors.WithMessage(err, "mysql query ommp_orderer fail")
	...
	ipNumPair := make(map[string]int, len(ipList))
	for _, ip := range ipList {
		indentSQL := "select count(*) from ommp_orderer where orderer_ip = \"" + ip + "\""
		rowN, err := db.Query(indentSQL)
		defer rowN.Close()
		if err != nil {
			return nil, errors.WithMessage(err, "mysql query ommp_orderer fail")
...
//GetServerIPListFromIndent 获取主机服务ip列表和节点个数
func GetServerIPListFromIndent(pageNum, pageSize int) ([]*ServerIpStruct, int,error) {
	var indentSQL string
	if pageNum == 0 {
	   indentSQL = "select distinct server_intip,server_extip from ommp_server where server_status = 1"
	}else{
		count := pageNum * pageSize
		pageNs := count - pageSize
		sqlS := "select distinct server_intip,server_extip from ommp_server where server_status = 1"
		indentSQL = fmt.Sprintf("%s order by id asc limit %d,%d", sqlS, pageNs, pageSize)
	}
	row, err := db.Query(indentSQL)
	defer row.Close()
	if err != nil {
		return nil, -1,errors.WithMessage(err, "mysql query ommp_server fail")
	}
	indentArrayMap, err := GetRowsValues(row)
	if err != nil {
		return nil, -1,errors.WithMessage(err, "mysql query ommp_server result rows to map fail")
	}
	//var ipList []string
	ipListInfo := make([]*ServerIpStruct,0)
	for _, ipL := range indentArrayMap {
        result := &ServerIpStruct{}
		ip := ipL["server_intip"]
		result.ServerIntIP = ip

		extip := ipL["server_extip"]
		result.ServerExtIP = extip
		//ipList = append(ipList, ip)
		// sqll := "select count(*) from ommp_indent where peer_ip = \"" + ip + "\""
		// rowN, err := db.Query(sqll)
		// defer rowN.Close()
		// if err != nil {
		...
		// 	n := numM["count(*)"]
		// 	num, _ := strconv.Atoi(n)
		// 	result.ServerNum = num
		// }
		ipListInfo = append(ipListInfo,result)
	}
	sqlTotal := "select count(*) from ommp_server where server_status = 1"
	total, err := GetPeerTotalNum(sqlTotal)
	if err != nil || total == -1 {
		return nil, -1, errors.WithMessage(err, "mysql query ommp_server  total num fail")
	}
	fmt.Println("get total num end")
	var pageTotal int
	if total <= pageSize {
		pageTotal = 1
	} else {
	...
	return ipListInfo,pageTotal, nil
}

//GetExtIPFromIntIP 根据内网IP获取主机外网ip列表和节点个
func GetExtIPFromIntIP(intIP string) (extip string,err error) {
	
	SelectSQL := "select server_extip from ommp_server where server_intip = \""+intIP+"\""
	
	row, err := db.Query(SelectSQL)
	defer row.Close()
	if err != nil {
		return "", errors.WithMessage(err, "mysql query ommp_server fail")
	}
	indentArrayMap, err := GetRowsValues(row)
	if err != nil {
...
//GetIntIPFromExtIP 根据外网IP获取主机内网ip列表和节点个
func GetIntIPFromExtIP(extIP string) (intip string,err error) {
	
	SelectSQL := "select server_intip from ommp_server where server_extip = \""+extIP+"\""
	...
	//var ipList []string
	for _, ipL := range indentArrayMap {
		intip = ipL["server_intip"]
	}
	return intip,nil
}

//GetUsingServerAllInfoFromIndent 获取所有在用主机服务所有信息
func GetUsingServerAllInfoFromIndent(pageNum, pageSize int) ([]*objectdefine.IndentServer, int,error) {
	var indentSQL string
	if pageNum == 0 {
		indentSQL = "select * from ommp_server where server_status = 1"
	}else{
		count := pageNum * pageSize
		pageNs := count - pageSize
		sqlS := "select * from ommp_server where server_status = 1"
		indentSQL = fmt.Sprintf("%s order by id asc limit %d,%d", sqlS, pageNs, pageSize)
	}
	row, err := db.Query(indentSQL)
	defer row.Close()
	...
	indentServerInfo := make([]*objectdefine.IndentServer, 0)
	for _, allInfo := range indentArrayMap {
		result := &objectdefine.IndentServer{}
        result.ServerName=allInfo["server_name"]
		result.ServerDes=allInfo["server_des"]
		result.ServerExtIp=allInfo["server_extip"]
		result.ServerIntIp=allInfo["server_intip"]
		result.ServerUser=allInfo["server_user"]
		result.ServerPassword=allInfo["server_password"]
		result.ServerStatus,_=strconv.Atoi(allInfo["server_status"])
		indentServerInfo = append(indentServerInfo,result)
	}
	
	// for _, server := range indentServerInfo {
	// 	indentSQL := "select count(*) from ommp_indent where peer_ip = \"" + server.ServerIntIp + "\""
	// 	rowN, err := db.Query(indentSQL)
	// 	defer rowN.Close()
	// 	if err != nil {
	// 		return nil, -1,errors.WithMessage(err, "mysql query ommp_indent fail")
	...
	return indentServerInfo, pageTotal,nil
}

//GetALLServerInfoFromIndent 获取主机服务所有信息
func GetALLServerInfoFromIndent(pageNum, pageSize int) ([]*objectdefine.IndentServer, int,error) {
	var indentSQL string
	if pageNum == 0 {
		indentSQL = "select * from ommp_server"
	}else{
		count := pageNum * pageSize
		pageNs := count - pageSize
		sqlS := "select * from ommp_server"
		indentSQL = fmt.Sprintf("%s order by id asc limit %d,%d", sqlS, pageNs, pageSize)
	}
	row, err := db.Query(indentSQL)
	defer row.Close()
