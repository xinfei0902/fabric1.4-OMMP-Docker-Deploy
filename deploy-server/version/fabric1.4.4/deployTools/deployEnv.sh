...
if [ $? -ne 0 ]; then
    ...
    echo "export GOPATH=/root/blockChain/gopath" >>/etc/profile
    ...
    mkdir -p /root/blockChain/gopath/src
    mkdir -p /root/blockChain/gopath/pkg
    mkdir -p /root/blockChain/gopath/bin
    echo "安装golang成功"
else
    echo "golang 环境已经安装"   
fi
