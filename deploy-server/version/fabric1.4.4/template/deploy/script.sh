...
#创建通道
CREATECHANNEL_MSPID=$[createChannelMSPID]
CREATECHANNEL_TLSROOT=$[createChannelTlsRoot]
CREATECHANNEL_MSP=$[createChannelMsp]
CREATECHANNEL_PEERADDRESS=$[createChannelPeerAddress]

ORG_NAME=$[orgNameArray]
PEER_CONFIG=$[peerConfigArray]
# 是否需要安装合约 on是 off 否
ISINSTALLCC=$[isInstallCC] 
...
CORE_PEER_LOCALMSPID=$CREATECHANNEL_MSPID
CORE_PEER_TLS_ROOTCERT_FILE=$CREATECHANNEL_TLSROOT
CORE_PEER_MSPCONFIGPATH=$CREATECHANNEL_MSP
CORE_PEER_ADDRESS=$CREATECHANNEL_PEERADDRESS
set -x
peer channel create -o $ORDERER_ADDRESS -c $CHANNEL_NAME -f ./channel-artifacts/$channelName.tx --tls true --cafile $ORDERER_TLSCA >&log.txt
res=$?
set +x
