...
#脚本后台运行命令
#nohup ./peer-status-monitor-anhui-admin.sh >/dev/null 2>&1 &
...
#INIT_STATUS 启动peer初始状态 0=停止 1=运行
INIT_STATUS=1
...
peerSuccessInfo='token=fb3f261e82a744249d8ef3a7173dd32d&name=$[peerNick]&accessKey=$[accessKey]&id=$[peerID]&ip=$[peerIP]&port=$[peerPort]&cityId=101260102&localion=106.65°E,26.68°N&status=1'
peerErrorInfo='token=fb3f261e82a744249d8ef3a7173dd32d&name=$[peerNick]&accessKey=$[accessKey]&id=$[peerID]&ip=$[peerIP]&port=$[peerPort]&cityId=101260102&localion=106.65°E,26.68°N&status=0'

putCheckSuccessMessage(){
 url="http://${ADDRESS}:${PORT}${URL}"
 #code=$(curl -H "Content-Type:application/json" -X POST --data ${peerSuccessInfo} $url |jq '.code')
 code=$(curl -X POST --data ${peerSuccessInfo}  $url |jq '.code')
 if [ $code -eq 200 ]; then
    echo success $code
 fi
}
