...
func GeneralCreatePeerScriptFile(general *objectdefine.Indent, orgOrder objectdefine.OrgType, peerOrder objectdefine.PeerType, output *objectdefine.TaskNode) error {
	...
	peerHostList := fmt.Sprintf("\"%s %s\"", ordererIP, ordererDomain)
	peerHostList = peerHostList + " " + fmt.Sprintf("\"%s %s\"", peerOrder.IP, peerOrder.Domain)
	exchangeMap["peerHosts"] = fmt.Sprintf("(%s)", peerHostList)
	//创建三个脚本文件
